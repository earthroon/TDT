...
export const OUT = path.join(ROOT, 'artifacts/resample-runtime-01-r12a/source-bake');
...
export function capture(id, fn) { try { return { id, status: 'PASS', evidence: fn() ?? true }; } catch (error) { return { id, status: 'FAIL', errorCode: String(error?.code || 'E_R12A_FINAL_RECEIPT_INCOMPLETE'), message: String(error?.message || error), detail: error?.detail ?? null }; } }
