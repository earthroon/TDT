...
export const OUT = path.join(ROOT, 'artifacts/resample-runtime-01-r13a/source-bake');
...
export function atomicWrite(file,data) { ensureDir(path.dirname(file)); const temp=`${file}.tmp-${process.pid}`; const fd=fs.openSync(temp,'w'); try{fs.writeFileSync(fd,data);fs.fsyncSync(fd);}finally{fs.closeSync(fd);} fs.renameSync(temp,file); }
export function sourceArtifact(name,value){ensureDir(OUT);atomicWrite(path.join(OUT,name),canonicalJson(value));return value;}
export function capture(id,fn){try{return{id,status:'PASS',evidence:fn()??true};}catch(error){return{id,status:'FAIL',errorCode:String(error?.code||'E_R13A_FINALIZER_REVALIDATION_FAILED'),message:String(error?.message||error),detail:error?.detail??null};}}
