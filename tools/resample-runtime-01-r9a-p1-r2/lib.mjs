...
export const OUT=path.join(ROOT,'artifacts/resample-runtime-01-r9a-p1-r2/source-bake');
...
export function canonical(value){if(Array.isArray(value))return value.map(canonical);if(value&&typeof value==='object')return Object.fromEntries(Object.keys(value).sort().map(k=>[k,canonical(value[k])]));if(typeof value==='number')check(Number.isFinite(value),'E_R9AP1R2_NONFINITE','nonfinite number');return Object.is(value,-0)?0:value;}
...
export function capture(id,fn){try{return{id,status:'PASS',evidence:fn()??true};}catch(error){return{id,status:'FAIL',errorCode:String(error?.code||'E_R9AP1R2_SOURCE_FAILURE'),message:String(error?.message||error),detail:error?.detail??null};}}
export async function captureAsync(id,fn){try{return{id,status:'PASS',evidence:(await fn())??true};}catch(error){return{id,status:'FAIL',errorCode:String(error?.code||'E_R9AP1R2_SOURCE_FAILURE'),message:String(error?.message||error),detail:error?.detail??null};}}
export function expectError(code,fn){try{fn();}catch(error){check(error?.code===code,'E_R9AP1R2_NEGATIVE_WRONG_CODE','unexpected error code',{expected:code,actual:error?.code});return true;}throw Object.assign(new Error('negative control did not fail'),{code:'E_R9AP1R2_NEGATIVE_DID_NOT_FAIL',detail:{expected:code}});}
