...
export const ARTIFACT_DIR=path.join(ROOT,'artifacts/resample-runtime-01-r1b/source-bake');
export const PATCH_ID='TDT-RESAMPLE-RUNTIME-01-R1B';
...
export function* walk(rel){const base=path.join(ROOT,rel);if(!fs.existsSync(base))return;for(const e of fs.readdirSync(base,{withFileTypes:true})){const child=path.posix.join(rel,e.name);if(e.isDirectory())yield* walk(child);else if(e.isFile())yield child;}}
